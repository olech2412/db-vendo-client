...
public class ArrivalsByStopIdRequestTest {
    ...
    public void testConstructorAndGetters() {
        ...
        assertNull("arrivalTime sollte standardmäßig null sein", request.getArrivalTime());
    ...
    public void testSettersAndGetters() {
        ArrivalsByStopIdRequest request = new ArrivalsByStopIdRequest("stop1");

        // Test für stopId
        request.setStopId("stop2");
        assertEquals("stopId sollte korrekt gesetzt werden", "stop2", request.getStopId());

        // Test für arrivalTime
        OffsetDateTime now = OffsetDateTime.now();
        request.setArrivalTime(now);
        assertEquals("arrivalTime sollte korrekt gesetzt werden", now, request.getArrivalTime());
    ...
    public void testNullValues() {
        ArrivalsByStopIdRequest request = new ArrivalsByStopIdRequest("test");

        request.setStopId(null);
        assertNull("stopId sollte null sein können", request.getStopId());

        request.setArrivalTime(null);
        assertNull("arrivalTime sollte null sein können", request.getArrivalTime());
